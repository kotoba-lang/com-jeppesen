(ns jeppesen.main
  "Kotodama WASM entrypoint for the Jeppesen clean-room actor (L4) — Clojure port.
  ...
  facts (`jeppesen.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "jeppesen")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"jeppesen.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jeppesen-compat" :tier tier :entities entities} 200])
...
(defn start! [] :jeppesen-compat/ready)
